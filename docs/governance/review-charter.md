...
4. Approvals were given on the final revision, or on an earlier revision with the same net diff (section 3). A push that changes the net diff voids earlier approvals; a push that only merges the base branch in, or rebases onto it, without changing the net diff does not.
...
- **No self-review.** You do not approve a pull request you authored, co-authored, or pushed to, whatever account you use. Writing a commit or being named in its Co-authored-by trailer always counts. Committing counts too, except for a commit that merged the base branch in, or came out of a force push that rewrote a history you had not contributed to, and in either case left the net diff unchanged: keeping a branch current is not writing it.
- **Net diff.** The net diff of a revision is what GitHub's three-dot compare of the base branch with that revision shows. Its fingerprint is a SHA-256 over each changed file's name, previous name, status and patch, with the line numbers removed from the hunk headers, so a base branch that moves underneath the change does not alter it. Two revisions with equal fingerprints carry the same change. When a fingerprint cannot be computed - GitHub truncated the compare, a file has no patch because it is binary or too large, or the API call failed - it matches nothing, and each rule falls back to the head commit: approvals count only on the head, the committer counts as having pushed, and the objection window runs from the last push.
...
- **Adopted pull requests (until 2026-10-05).** A pull request the maintainer has pushed to - a fix, a test, a merge from `main` - and then declared adopted in the thread merges as the maintainer's own change under the bullet above: no further approvals, a *changes requested* from any committer or core reviewer still blocks it, and the objection window, the queue rules and the rules on this page that do not count approvals apply unchanged. The contributor stays the author of the change. The declaration is a review on the current head reading `Adopted by the maintainer: whole diff read, fixes pushed, CI green.`; a later push needs a new one, even one that leaves the net diff unchanged. Adoption applies only to a pull request that needs the ordinary two approvals - not one over 400 changed lines or touching a `sandbox`, `security` or `audit` path - that touches no path with a CODEOWNERS line of its own, and that touches nothing under `.github/`, `schemas/` or `proto/`. Anywhere else the declaration is ignored and the pull request needs its approvals. The `quorum` check reads the maintainer's commit and the notice, and applies the date itself; this bullet is removed once it has passed.
...
This page changes by pull request. It is a protected path, so the maintainer must approve; in addition the pull request stays open for 72 hours after the last push that changed its net diff (section 3) so committers can object; a push that only merges the base branch in or rebases onto it does not restart the window, and when the net diff cannot be compared the window runs from the last push. The same window applies to `GOVERNANCE.md`, `MAINTAINERS.md`, `.github/CODEOWNERS`, `.github/quorum-roster.toml`, `scripts/quorum_check.py` and `scripts/queue_hygiene.py`. Objections are answered in the thread before merge.
