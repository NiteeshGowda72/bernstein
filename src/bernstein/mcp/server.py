...
#: infer the default.
_TOOL_TASK_SUPPORT: dict[str, typing.Literal["forbidden", "optional", "required"]] = {
...
def _shape_tools_list(mcp: MCPServer[None]) -> None:
    ...
    * ``outputSchema`` is stamped for the tools that return structured
    ...
    mcp._lowlevel_server.add_request_handler('tools/list', ListToolsRequest.model_fields['params'].annotation, lambda ctx, params: shaped_list_tools())
...
def _register_tasks_extension(mcp: MCPServer[None], server_url: str) -> None:
    from mcp.types import GetTaskRequest, ListTasksRequest, CancelTaskRequest
...
def create_mcp_server(
    ...
    *,
    ...
    mcp: MCPServer[None] = MCPServer(name, instructions=_SERVER_INSTRUCTIONS, version=_package_version())
